---
title: "GSM-сигнализация для дачи своими руками: как установить"
slug: "gsm-signalizaciya-dlya-dachi-svoimi-rukami"
seo_title: "GSM-сигнализация для дачи своими руками: установка"
seo_description: "GSM-сигнализация для дачи своими руками: как работает, какие датчики нужны и как её установить без прокладки кабеля и без охранной компании. Калькулятор зон."
focus_keyword: "gsm сигнализация для дачи своими руками"
...
status: draft
images:
  - images/gsm-signalizaciya-dlya-dachi-svoimi-rukami.jpg
  - images/gsm-signalizaciya-dlya-dachi-svoimi-rukami-2.jpg
  - images/gsm-signalizaciya-dlya-dachi-svoimi-rukami-3.jpg
  - images/gsm-signalizaciya-dlya-dachi-svoimi-rukami-4.jpg
  - images/gsm-signalizaciya-dlya-dachi-svoimi-rukami-5.jpg
  - images/gsm-signalizaciya-dlya-dachi-svoimi-rukami-6.jpg
---
...
![Базовый блок GSM-сигнализации на стене дачного дома](images/gsm-signalizaciya-dlya-dachi-svoimi-rukami.jpg)
...
![Магнитный датчик на оконной раме дачного дома](images/gsm-signalizaciya-dlya-dachi-svoimi-rukami-2.jpg)
...
Тот же довод в пользу GSM работает и для удалённого управления: она не
зависит от домашнего Wi-Fi и работает по SMS, тогда как умная розетка для
отопления отваливается от сети именно тогда, когда домашний интернет
слабый или на морозе — подробный разбор минусов в статье
...
![Комплект GSM-сигнализации: базовый блок и датчики на столе](images/gsm-signalizaciya-dlya-dachi-svoimi-rukami-3.jpg)
...
![Установка ИК-датчика движения на стене внутри дачного дома](images/gsm-signalizaciya-dlya-dachi-svoimi-rukami-4.jpg)

![Настройка номеров оповещения в приложении сигнализации на смартфоне](images/gsm-signalizaciya-dlya-dachi-svoimi-rukami-5.jpg)
...
<!-- wp:html -->
<div id="mdGsCalc" class="md-gs">
<div class="md-gs__head">
<h3 class="md-gs__title">Калькулятор зон GSM-сигнализации</h3>
<p class="md-gs__sub">Каждая дверь, окно и отдельная зона движения — своя зона на панели.</p>
</div>
<div class="md-gs__grid">
<label class="md-gs__f"><span>Входных дверей</span><input type="number" id="gsDoors" value="2" min="0" max="20" step="1"></label>
<label class="md-gs__f"><span>Окон первого этажа под датчик</span><input type="number" id="gsWindows" value="4" min="0" max="30" step="1"></label>
<label class="md-gs__f"><span>ИК-датчиков движения внутри</span><input type="number" id="gsMotion" value="1" min="0" max="10" step="1"></label>
<label class="md-gs__f"><span>Уличных зон/барьеров по периметру</span><input type="number" id="gsOutdoor" value="0" min="0" max="10" step="1"></label>
<label class="md-gs__f"><span>Хозпостроек под охрану (сарай, баня и т.п.)</span><input type="number" id="gsOut" value="0" min="0" max="10" step="1"></label>
</div>
<div class="md-gs__res">
<div class="md-gs__resitem"><span>Всего зон</span><b id="gsTotal">7</b></div>
<div class="md-gs__resitem"><span>Панель</span><b id="gsPanel">—</b></div>
</div>
<p id="gsNote" class="md-gs__note"></p>
<div class="md-gs__actions">
...
<p id="gsStatus" class="md-gs__status"></p>
</div>
<style>
.md-gs{--gs-g:#a04747;--gs-bd:#e2e2e2;--gs-bg:#faf5f5;--gs-vk:#0077ff;--gs-tg:#26a5e4;max-width:760px;margin:28px auto;padding:20px;border:1px solid var(--gs-bd);border-radius:14px;background:#fff;font-family:inherit;color:#222;line-height:1.5;box-sizing:border-box}
.md-gs *{box-sizing:border-box}
.md-gs__title{margin:0 0 6px;font-size:1.3em;color:var(--gs-g)}
.md-gs__sub{margin:0 0 16px;font-size:.9em;color:#666}
.md-gs__grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px;margin-bottom:10px}
.md-gs__f{display:flex;flex-direction:column;gap:5px;font-size:.88em;font-weight:600;color:#444}
.md-gs__f input{padding:10px 12px;border:1px solid var(--gs-bd);border-radius:9px;font-size:16px;font-family:inherit;font-weight:400;background:#fafafa;width:100%}
.md-gs__f input:focus{outline:none;border-color:var(--gs-g);background:#fff}
.md-gs__res{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;padding:14px;background:var(--gs-bg);border-radius:10px;margin-bottom:6px}
.md-gs__resitem{text-align:center}
.md-gs__resitem span{display:block;font-size:.8em;color:#666;margin-bottom:3px}
.md-gs__resitem b{font-size:1.15em;color:var(--gs-g)}
.md-gs__note{margin:0 0 10px;font-size:.83em;color:#888;font-style:italic}
.md-gs__actions{display:flex;gap:9px;margin-top:4px;flex-wrap:wrap}
.md-gs__btn{flex:1 1 auto;min-width:130px;padding:12px 16px;border:1px solid var(--gs-g);border-radius:9px;background:#fff;color:var(--gs-g);font-size:.95em;font-family:inherit;font-weight:600;cursor:pointer;text-align:center;text-decoration:none;display:inline-block}
.md-gs__btn--main{background:var(--gs-g);color:#fff}
.md-gs__btn--vk{border-color:var(--gs-vk);color:var(--gs-vk)}
.md-gs__btn--tg{border-color:var(--gs-tg);color:var(--gs-tg)}
.md-gs__btn:active{opacity:.7}
.md-gs__status{margin:10px 0 0;font-size:.87em;color:var(--gs-g);min-height:1.2em;text-align:center}
@media (max-width:600px){.md-gs{padding:15px;border-radius:11px}.md-gs__grid{grid-template-columns:1fr}}
@media print{.md-gs__actions,.md-gs__status,.md-gs__sub{display:none}.md-gs{border:none;max-width:100%}}
</style>
<script>
(function(){
var $=function(id){return document.getElementById(id)};
function calc(){
 var doors=parseInt($('gsDoors').value)||0;
 var windows=parseInt($('gsWindows').value)||0;
 var motion=parseInt($('gsMotion').value)||0;
 var outdoor=parseInt($('gsOutdoor').value)||0;
 var out=parseInt($('gsOut').value)||0;
 var total=doors+windows+motion+outdoor+out;
 var panel, note;
 if(total<=8){panel='базовая, на 8 зон';note='Хватит стартового комплекта без докупки расширителя зон.';}
 else if(total<=16){panel='расширенная, на 16 зон';note='Нужна панель с поддержкой минимум 16 зон или радиорасширитель к базовой.';}
 else{panel='профессиональная, от 32 зон';note='Столько зон обычно требует модульная система, а не готовый бытовой комплект.';}
 $('gsTotal').textContent=total;
 $('gsPanel').textContent=panel;
 $('gsNote').textContent=note;
 return {total:total,panel:panel,doors:doors,windows:windows,motion:motion,outdoor:outdoor,out:out};
}
function shortText(){
 var d=calc();
 return 'Дверей '+d.doors+', окон '+d.windows+', ИК-датчиков '+d.motion+', уличных зон '+d.outdoor+', построек '+d.out+' — всего '+d.total+' зон, нужна панель: '+d.panel+'. Расчёт: mir-doma.pro/gsm-signalizaciya-dlya-dachi-svoimi-rukami/';
}
function updateShareLinks(){
 var url=location.href.split('#')[0];
 var text=shortText();
 $('gsVk').href='https://vk.com/share.php?url='+encodeURIComponent(url)+'&title='+encodeURIComponent('Расчёт зон GSM-сигнализации')+'&description='+encodeURIComponent(text);
 $('gsTg').href='https://t.me/share/url?url='+encodeURIComponent(url)+'&text='+encodeURIComponent(text);
}
function recalc(){ calc(); updateShareLinks(); }
['gsDoors','gsWindows','gsMotion','gsOutdoor','gsOut'].forEach(function(id){
 var el=$(id); el.addEventListener('input',recalc); el.addEventListener('change',recalc);
});
function flash(msg){$('gsStatus').textContent=msg;setTimeout(function(){$('gsStatus').textContent='';},2600);}
$('gsCopy').addEventListener('click',function(){
 var t=shortText();
...
if(navigator.share){
 $('gsShareNative').hidden=false;
 $('gsShareNative').addEventListener('click',function(){
  navigator.share({title:'Расчёт зон GSM-сигнализации',text:shortText(),url:location.href.split('#')[0]}).catch(function(){});
 });
}
$('gsPrint').addEventListener('click',function(){window.print();});
recalc();
})();
</script>
<!-- /wp:html -->
...
![Тестирование зоны GSM-сигнализации со смартфоном в руке](images/gsm-signalizaciya-dlya-dachi-svoimi-rukami-6.jpg)
