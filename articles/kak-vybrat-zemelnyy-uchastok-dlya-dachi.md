...
<!-- wp:html -->
<div id="mdPriceMap" class="md-pm">
  <p class="md-pm__lead">Цена сотки по направлениям от Москвы — нажмите на направление, чтобы увидеть города</p>
  <div id="mdPmList" class="md-pm__list"></div>
  <p class="md-pm__note">Снимок рынка на конкретную дату — ориентир для сравнения направлений, а не цена вашего участка. Источники и точные ссылки — под таблицей.</p>
</div>
<style>
.md-pm{max-width:680px;margin:24px auto;padding:18px 20px;border:1px solid #e2e2e2;border-radius:12px;background:#fff}
.md-pm__lead{margin:0 0 14px;font-size:.95em;color:#555}
.md-pm__row{margin-bottom:9px;cursor:pointer}
.md-pm__rowhead{display:flex;align-items:center;gap:10px;font-size:.93em}
.md-pm__name{flex:0 0 168px;font-weight:600;color:#333}
.md-pm__barwrap{flex:1;height:20px;background:#f0f0f0;border-radius:6px;overflow:hidden}
.md-pm__bar{height:100%;border-radius:6px}
.md-pm__price{flex:0 0 88px;text-align:right;font-variant-numeric:tabular-nums;color:#333}
.md-pm__detail{max-height:0;overflow:hidden;transition:max-height .2s ease;font-size:.86em;color:#666;padding-left:178px}
.md-pm__row.is-open .md-pm__detail{max-height:60px;padding-top:6px}
.md-pm__note{margin:14px 0 0;font-size:.82em;color:#888;line-height:1.4}
@media (max-width:560px){
  .md-pm__name{flex-basis:110px;font-size:.9em}
  .md-pm__price{flex-basis:70px;font-size:.9em}
  .md-pm__detail{padding-left:120px}
}
</style>
<script>
(function(){
  var DATA=[
    {n:'Егорьевское шоссе',p:97600,c:'через Раменское, Бронницы, Воскресенск, Егорьевск'},
    {n:'Новорязанское шоссе',p:107900,c:'через Люберцы, Бронницы, Воскресенск'},
    {n:'Горьковское шоссе',p:127900,c:'через Балашиху, Ногинск, Павловский Посад'},
    {n:'Каширское шоссе',p:187700,c:'через Домодедово, Ступино, Каширу'},
    {n:'Симферопольское шоссе',p:198500,c:'через Подольск, Чехов, Серпухов'},
    {n:'Дмитровское направление',p:484000,c:'Дмитровский округ, данные за май 2026'},
    {n:'Новорижское шоссе',p:668700,c:'через Красногорск, Истру, Волоколамск'},
    {n:'Осташковское шоссе',p:791000,c:'через Мытищи, Королёв, Пирогово'},
    {n:'Боровское шоссе',p:2075000,c:'через Троицк, Апрелевку'},
    {n:'Ильинское шоссе',p:2519000,c:'ближнее Подмосковье у Рублёвки'},
    {n:'Рублёво-Успенское шоссе',p:3546000,c:'самое дорогое направление области'}
  ];
  var root=document.getElementById('mdPmList');
  var min=Math.log10(Math.min.apply(null,DATA.map(function(d){return d.p;})));
  var max=Math.log10(Math.max.apply(null,DATA.map(function(d){return d.p;})));
  function color(pct){
    if(pct<33) return '#3d7a3d';
    if(pct<66) return '#c9962b';
    return '#c0392b';
  }
...
<!-- wp:html -->
<div id="mdChecklistLand" class="md-cl">
  <div class="md-cl__head">
    <h3 class="md-cl__title">Чек-лист выбора участка</h3>
    <p class="md-cl__sub">Отмечайте пункты прямо на месте — всё сохранится в браузере на этом устройстве.</p>
    ...
    <div class="md-cl__progwrap">
      <div class="md-cl__progbar"><div id="mdProgFillLand" class="md-cl__progfill"></div></div>
      <span id="mdProgTextLand" class="md-cl__progtext">0 из 0</span>
    </div>
  </div>
  <div id="mdSectionsLand" class="md-cl__body"></div>
  ...
  <p id="mdStatusLand" class="md-cl__status"></p>
</div>
<style>
...
<script>
(function(){
  var KEY='mdLandChecklist_v1';
  var DATA=[
    {t:'Документы',h:'Проверяется до задатка, а не после',i:[
      ...
      'Основание права собственности продавца и срок владения проверены'
    ]},
    {t:'СНТ / ДНП',h:'Вместе с землёй покупается ещё и членство в товариществе',i:[
    ...
    {t:'Соседи и окружение',h:'То, что не купишь и не исправишь после сделки',i:[
    ...
    {t:'Коммуникации',h:'Того, что «по границе», может не быть в реальности',i:[
  ...
  var root=document.getElementById('mdSectionsLand');
  var addr=document.getElementById('mdAddrLand');
  var notes=document.getElementById('mdNotesLand');
  var fill=document.getElementById('mdProgFillLand');
  var ptext=document.getElementById('mdProgTextLand');
  var status=document.getElementById('mdStatusLand');
  var total=0;
  ...
  function refresh(){
    ...
    DATA.forEach(function(sec,si){
      ...
      sec.i.forEach(function(_,ii){
        var cb=document.getElementById('mdl_'+si+'_'+ii);
        if(cb&&cb.checked) d++;
      });
  ...
  function report(){
    var out='ЧЕК-ЛИСТ ВЫБОРА УЧАСТКА\n';
  ...
  document.getElementById('mdShareLand').addEventListener('click',function(){
    var txt=report();
    if(navigator.share){
      navigator.share({title:'Чек-лист выбора участка',text:txt}).catch(function(){});
    }else{
      var blob=new Blob([txt],{type:'text/plain;charset=utf-8'});
      var a=document.createElement('a');
      a.href=URL.createObjectURL(blob);
      a.download='vybor-uchastka-'+(new Date().toISOString().slice(0,10))+'.txt';
      document.body.appendChild(a); a.click(); document.body.removeChild(a);
      setTimeout(function(){ URL.revokeObjectURL(a.href); },1000);
      flash('Файл сохранён');
    }
  });
  document.getElementById('mdCopyLand').addEventListener('click',function(){
    var txt=report();
  ...
  document.getElementById('mdPrintLand').addEventListener('click',function(){ window.print(); });
  document.getElementById('mdResetLand').addEventListener('click',function(){
